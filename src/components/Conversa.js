...
import { Image, ListView, StyleSheet, Text, TextInput, TouchableHighlight, View, ImageBackground } from 'react-native';
        ...
        if (texto.tipo === 'e') {
            return (
                <View style={{ alignItems: 'flex-end', marginTop: 5, marginBottom: 5, marginLeft: 40 }}>
                    <Text style={{ fontSize: 18, color: '#000', padding: 10, backgroundColor: '#dbf5b4', elevation: 1 }}>{texto.tipo} -> {texto.mensagem}</Text>
                </View>
            );
        } else {
            return (
                <View style={{ alignItems: 'flex-start', marginTop: 5, marginBottom: 5, marginRight: 40 }}>
                    <Text style={{ fontSize: 18, color: '#000', padding: 10, backgroundColor: '#f7f7f7', elevation: 1 }}>{texto.tipo} -> {texto.mensagem}</Text>
                </View>
            );
        }
    }
